-- Make collected totals obey the same dashboard dimensional filters as sales.
-- Payments are scoped through their linked booking first, then their order.

create index if not exists payment_dashboard_booking_idx
  on payment (organization_id, booking_id, paid_at);

create index if not exists payment_dashboard_order_idx
  on payment (organization_id, order_id, paid_at);
