-- Panel ejecutivo — correcciones de severidad ALTA (auditoría).
-- A2: el margen de series y rankings debe ser margen de CONTRIBUCIÓN (ventas
--     netas − costos − comisiones variables atribuibles), coherente con el KPI.
--     Antes usaba sale_base − cost_base (sin comisión), produciendo dos
--     definiciones de "margen" en la misma pantalla.
-- A3: comisiones / cuentas por cobrar / por pagar en moneda extranjera no tienen
--     columna base ni tipo de cambio, así que su monto se excluye del total. El
--     conteo, en cambio, las incluía → "Comisiones: $0 · 3". Ahora el conteo solo
--     cuenta las filas realmente sumadas (moneda base) y se expone
--     `*_excluded_count` para revelar las que quedan sin conversión.
-- (A1 se resuelve en la capa API/UI, no aquí.)
...
begin
  ...
  select jsonb_build_object(
    ...
    'upcoming_departures', coalesce((select jsonb_agg(to_jsonb(x)) from (select * from upcoming_rows order by occupancy asc, departure_at asc limit 8) x), '[]'::jsonb)
  ) into out
