-- Panel ejecutivo — correcciones de severidad MEDIA (auditoría).
-- M1: "cuentas por cobrar vencidas" se contaban por status = 'overdue', un flag
--     que puede quedar obsoleto. Ahora se derivan del vencimiento real
--     (due_date < hoy en la zona de la organización) sobre las CxC abiertas.
-- M2: "Efectivo en caja" sumaba divisas distintas a un único total base, lo que
--     oculta cuánto hay físicamente en cada moneda. Se añade
--     `cash_by_currency` (monto y sesiones por divisa) sin quitar el total base.
-- M3: "Próximas salidas" se ordenaba por menor ocupación; una lista de próximas
--     salidas debe ir en orden cronológico. La ocupación crítica ya se comunica
--     por las alertas ejecutivas. Ahora ordena por fecha de salida ascendente.
-- M5: la caja (cajero) veía la recaudación de toda la organización. Ahora
--     `p_cash_user_id` también acota los pagos a los registrados por ese usuario.
-- (Reemplaza la definición de 0026, conservando A1/A2/A3.)
