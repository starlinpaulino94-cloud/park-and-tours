-- P-001 · El panel ejecutivo dejaba de caber en memoria.
--
-- Medido sobre 120.000 reservas y 60.000 cobros de un mismo inquilino: una
-- ventana de 365 días tardaba ~800 ms. El plan decía por qué: `current_booking`
-- se definía como `select b.*, ...`, de modo que la CTE materializaba las ~60
-- columnas de `booking` (1.247 bytes por fila, ~150 MB) en un archivo temporal,
-- y los seis agregados que la leen —resumen, serie diaria, canal, producto,
-- vendedor y socio— la releían entera cada uno: 20.065 bloques temporales
-- leídos por llamada, unos 160 MB de E/S en disco dentro de una sola consulta.
--
-- Ninguno de esos seis consumidores necesita `b.*`: entre todos usan siete
-- columnas de la tabla más los cuatro valores derivados. Lo mismo ocurría en
-- las otras siete CTE base. Esta migración sólo recorta la lista de columnas
-- proyectada; la aritmética, los filtros y el JSON de salida son los de 0028,
-- byte por byte. (Reemplaza la definición de 0028, conservando A1/A2/A3 y
-- M1/M2/M3/M5 y B4.)
