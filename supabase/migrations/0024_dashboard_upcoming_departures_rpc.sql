-- Move the dashboard's upcoming departures into the dashboard_summary RPC so the
-- executive panel is backed by a single tenant-checked database round-trip.
