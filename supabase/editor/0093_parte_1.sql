-- 0093 parte 1 de 2 — el enganche del token, entero otra vez.
-- Pegar ENTERO en el editor SQL de Supabase y ejecutar. Luego la parte 2.
--
-- Qué arregla: 0084 reescribió el enganche partiendo de la versión de 0063 y
-- perdió dos cosas que se habían añadido entre medias, sin romper nada de forma
-- visible:
--
--   · la EMPRESA ACTIVA del selector (0068): desde 0084 el selector guarda la
--     elección, la pantalla la enseña, y el token sigue llevando la empresa
--     principal. Quien administra dos operadoras cambia de empresa y trabaja
--     sobre la otra creyendo que cambió.
--   · la SUCURSAL (0046): `branch_id` dejó de viajar, así que `ctx.branchId` es
--     nulo para todo el mundo desde entonces.
--
-- IMPORTANTE: después de ejecutar esto, CIERRA SESIÓN Y VUELVE A ENTRAR. El
-- token viejo sigue siendo válido hasta que caduca, y sigue sin los dos
-- atributos: el enganche solo corre al emitir uno nuevo.
