...
| ¿Puede hacerse el **cutover a Supabase** hoy? | **NO, absolutamente no.** `DB-001` (RLS nunca activada) expondría los datos de todos los tenants |
...
1. **La frontera de seguridad que la documentación da por hecha no existe.** `app.enable_tenant_rls()` está escrita y no se invoca en ninguna de las 77 tablas de negocio. La documentación previa afirma lo contrario. Un cutover basado en esa afirmación expondría todo.
...
| **B — Build** | ✅ **PASA** | Build ✅ · Typecheck ✅ limpio · Lint ✅ · CI en cada PR |
| **C — Base de datos** | ❌ **FALLA** | Constraints ✅ *en el esquema Postgres, sin usar* · Migraciones ⚠️ ficheros sueltos sin herramienta · Índices ⚠️ no justificados por `EXPLAIN` · **RLS ❌ `DB-001`** · Backup ❌ · Restore ❌ **nunca probado** · Esquema de producción (Totalum) **no está en el repositorio** |
| **D — Seguridad** | ❌ **FALLA** | Authorization ✅ (77/77 con `writeRole`, partner deny-by-default) · Secretos ✅ (árbol e historial limpios) · Auth ⚠️ (sin reset ni verificación de email; rate limit inoperante) · OWASP ❌ (`SEC-001`, `SEC-002`, `SEC-004`, `SEC-005`) · Dependencias ❌ (3 críticas) |
...
| Database | **35** | Esquema Postgres bien modelado (326 FK, 266 CHECK, 159 índices). Penaliza fuerte: **el motor en producción no tiene ninguna de esas garantías y su esquema no está en el repositorio** |
...
| Security | **48** | RBAC completo y verificado, sin secretos, sin XSS/SQLi, partner deny-by-default. Penaliza: `SEC-001`, `SEC-002`, clickjacking, CORS, 3 CVE críticas |
...
| Severidad | Nº | IDs |
|---|---:|---|
| **P0** | **3** | `SEC-001`/`DB-001` (RLS nunca activada) · `SEC-002`/`DB-002` (RPC ejecutable por `anon` sin comprobación de tenant) · `SEC-003` (CVE críticas en la librería de auth, coincidentes con esta configuración) |
...
Los tres P0 comparten una causa raíz y no son bugs independientes — ver `REMEDIATION_PLAN.md` §Causas raíz.
...
- Si existe ya un proyecto Supabase con datos reales — de ser así, `SEC-001` sería un incidente **activo**, no un riesgo futuro.
