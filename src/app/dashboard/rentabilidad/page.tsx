          ...
          <section className="tf-card p-5">
            ...
            <ul className="space-y-2 text-sm">
              ...
              <Waterfall label="Descuentos aplicados" value={`− ${money(t?.discounts)}`}
                pct={t?.gross ? ((t.discounts ?? 0) / t.gross) * 100 : 0} tone="bg-[oklch(0.79_0.14_80)]" />
              ...
              <Waterfall label="Comisiones" value={`− ${money(t?.commissions)}`}
                pct={t?.gross ? ((t.commissions ?? 0) / t.gross) * 100 : 0} tone="bg-[oklch(0.63_0.17_35)]" />
              <Waterfall label="Costes operativos" value={`− ${money(t?.costs)}`}
                pct={t?.gross ? ((t.costs ?? 0) / t.gross) * 100 : 0} tone="bg-[oklch(0.63_0.17_35)]" />
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: "costs", header: "Costes", align: "right", hideOn: "md", render: (r: any) => money(r.costs) },
              {
                ...
                render: (r: any) => (
                  <div>
                    <p className={r.margin >= 0 ? "font-semibold text-emerald-600 dark:text-emerald-400" : "font-semibold text-rose-600 dark:text-rose-400"}>
                      {money(r.margin)}
                    </p>
  ...
  return (
    <li>
      <div className="flex items-baseline justify-between gap-3">
        ...
        <span className={`tabular-nums ${strong ? "font-display text-base font-semibold" : "font-medium"}`}>{value}</span>
      </div>
